...
exports.readMany = async function readMany(req, res, next) {
  try {
    ...
    const query = (function buildQuery() {
       ...
       */
      const isSelfUserFromOrUserTo = [userFrom, userTo].includes(selfId);
    ...
    const isSelfUserFrom = userFrom === selfId;
...
exports.referenceById = async function referenceById(req, res, next, id) { // eslint-disable-line no-unused-vars
  try {
    ...
    const userFromId = (reference) ? reference.userFrom._id.toString() : null;
    const userToId = (reference) ? reference.userTo._id.toString() : null;

    // make sure that nonpublic references are not exposed
    // nonpublic reference can be exposed to userFrom or userTo only.
    const isExistentPublicOrFromToSelf = reference && (reference.public || [userFromId, userToId].includes(selfId));
